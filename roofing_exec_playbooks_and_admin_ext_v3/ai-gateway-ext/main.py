...
async def _emit_usage(route, qty, unit, meta):
    try:
        async with httpx.AsyncClient(timeout=1.0) as c:
            await c.post(f"{TELEMETRY_URL}/event", json={
                "service":"8023","route":route,"action":"llm_call","quantity":qty,"unit":unit,"meta":meta
            })
    except Exception:
        pass
...
async def chat(req: ChatRequest, request: Request, x_user_id: Optional[str] = Header(default="anon")):
    ...
    _day_cost += cost
    await _emit_usage("/chat", 1, "call", {"tokens": tokens, "user": x_user_id, "cost": cost})
